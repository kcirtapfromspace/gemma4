---
experiment: mlc-llm-jetson-orin-nx
team_tag: mlc
backend: mlc-llm
device: jetson-orin-nx
runtime: gpu
model_variant: gemma-4-E2B-base
model_format: mlc-q4f16
data_source: measured
created_at: 2026-04-19 12:00:00
status: done
---

# mlc-llm-jetson-orin-nx

<!-- METRICS:BEGIN (auto-generated from duckdb — edits here are overwritten) -->
## Metrics

- **avg_gen_tok_s**: 6.50  •  **p50_gen_tok_s**: —
- **avg_ttft_ms**: —  •  **avg_prompt_tok_s**: —
- **extraction_pass_rate**: —
- **avg_extraction_score**: —
- **success_rate (valid JSON)**: —
- **total_runs**: —

> MLC-LLM port (commit 8da77e9) on Jetson Orin NX: 5-8 tok/s — 6x over llama.cpp. Not production-ready (no LoRA, sequence degeneration).

<!-- METRICS:END -->

## Hypothesis
_TODO: what did we expect going in? What would make this experiment interesting?_

## Method
_TODO: hardware, workload, scorer, anything non-obvious about how this was measured._

## Result
_TODO: narrative interpretation of the Metrics block above. Surprises? Regressions?_

## Decision
_TODO: what are we doing with this? What's the next experiment?_

## Links
- duckdb: `SELECT * FROM experiments WHERE experiment_name = 'mlc-llm-jetson-orin-nx';`
- per-case: `SELECT * FROM benchmark_runs WHERE experiment_name = 'mlc-llm-jetson-orin-nx';`
- commit: _TODO_
- raw artifacts: _TODO_
